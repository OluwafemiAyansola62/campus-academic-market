...
export function Pricing(){
  ...
  function openCheckout(plan:Plan){if(!plan.price){document.getElementById('resources')?.scrollIntoView({behavior:'smooth'});return;}setError('');setEmail('');setSelected(plan);}
  async function pay(e:FormEvent){e.preventDefault();if(!selected)return;if(!/^\S+@\S+\.\S+$/.test(email)){setError('Enter a valid email address.');return;}setLoading(selected.name);setError('');try{const r=await fetch('/api/payments/initialize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({plan:selected.name,email})});const data=await r.json();if(!r.ok)throw new Error(data.error||'Payment could not start');window.location.href=data.link;}catch(err){setError(err instanceof Error?err.message:'Payment error');setLoading('');}}
  return <>
    <div className="pricing-grid">{plans.map((p)=><div key={p.name} className={'price-card card reveal '+(p.featured?'featured':'')}>
      {p.featured&&<div className="popular"><Sparkles size={13}/> MOST POPULAR</div>}<h3>{p.name}</h3><p>{p.desc}</p><div className="price">₦{p.price.toLocaleString()}<small>{p.period}</small></div>
      <ul>{p.features.map(f=><li key={f}><Check size={16}/>{f}</li>)}</ul>
      <button className={'btn '+(p.featured?'btn-primary':'btn-secondary')} onClick={()=>openCheckout(p)} disabled={!!loading}>{loading===p.name?<LoaderCircle className="spin" size={17}/>:p.cta}</button>
    </div>)}</div>
    {selected&&<div className="checkout-backdrop" role="dialog" aria-modal="true" aria-label="CAM checkout"><div className="checkout-card">
      <button className="close" onClick={()=>{setSelected(null);setLoading('')}} aria-label="Close checkout"><X size={18}/></button><span className="eyebrow">Secure CAM checkout</span>
      <h3>{selected.name}</h3><p>Enter your email to continue to Flutterwave. Your payment is processed securely on Flutterwave.</p><div className="checkout-price">₦{selected.price.toLocaleString()}<small>{selected.period}</small></div>
      <form onSubmit={pay}><label htmlFor="cam-email">Email address</label><input id="cam-email" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" required/>{error&&<div className="form-error" role="alert">{error}</div>}<button className="btn btn-primary" type="submit" disabled={!!loading}>{loading?<><LoaderCircle className="spin" size={17}/> Connecting to Flutterwave...</>:`Continue to payment`}</button></form>
